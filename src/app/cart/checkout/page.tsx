...
  zip_code?: string; // Assuming these might exist
  country?: string; // Assuming these might exist
  is_primary?: boolean; // Assuming this column exists for primary address
}

// Define a type for your product structure (assuming it's consistent)
interface Product {
  name: string;
  image: string;
  category: string;
...
// Define a type for a cart item, including the product details
interface CartItem {
  id: string;
  product_id: string;
  quantity: number;
...
  product: Product; // Ensure product is always present here based on your data fetching
}
...
  const [selectedPaymentMethod, setSelectedPaymentMethod] = useState<string>("cash_on_delivery"); // Default
...
          const primary = userAddresses.find(addr => addr.is_primary) || userAddresses[0];
...
      showAppNotification("Your cart is empty. Cannot place an empty order.", "error");
      return;
    }
...
    try {
      const deliveryAddressString = `${selectedAddress.street_address}, ${selectedAddress.city}, ${selectedAddress.state_province}${selectedAddress.zip_code ? `, ${selectedAddress.zip_code}` : ''}${selectedAddress.country ? `, ${selectedAddress.country}` : ''}`;
...
          p_delivery_name: selectedAddress.full_name,
          p_delivery_phone: selectedAddress.phone_number,
...
      if (!newOrderId) {
        throw new Error("Order creation failed - no order ID returned from Supabase.");
      }
...
      showAppNotification("Order placed successfully! Redirecting to orders...", "success");
...
                  <p className="text-gray-700">Pay with cash when your order arrives.</p>
                </div>
              </label>
...
            disabled={checkoutProcessing || !selectedAddress || items.length === 0}
...
              via{" "}
              <span className="font-semibold text-gray-900">
                {selectedPaymentMethod === "cash_on_delivery" ? "Cash on Delivery" : selectedPaymentMethod}.
              </span>
            </p>
