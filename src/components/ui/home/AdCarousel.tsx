...
export function AdCarousel({
  ...
  const prevButtonClass = `ad-carousel-prev-${navigationId}`;
  const nextButtonClass = `ad-carousel-next-${navigationId}`;
  ...
  return (
    <div className="relative group max-w-7xl mx-auto">
      ...
      <button
        className={`${prevButtonClass} absolute top-1/2 left-4 z-20 -translate-y-1/2 flex items-center justify-center w-10 h-10 bg-white/30 hover:bg-white/60 text-white rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-75 backdrop-blur-sm`}
        aria-label="Previous advertisement"
      >
        <FontAwesomeIcon icon={faChevronLeft} className="text-xl" />
      </button>
      <button
        className={`${nextButtonClass} absolute top-1/2 right-4 z-20 -translate-y-1/2 flex items-center justify-center w-10 h-10 bg-white/30 hover:bg-white/60 text-white rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-75 backdrop-blur-sm`}
